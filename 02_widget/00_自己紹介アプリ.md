...
### **演習では細かい説明はしませんが気になる人は解説の部分も読んでみよう**
...
解説
- `runApp`の引数（括弧の中身）に画面に表示する部品組み立てたものを設定
- 後続のclassで組み立てて、最後に`runApp`が実行されるイメージ
...
- `MaterialApp`:マテリアルデザインに準拠したwidgetを提供してくれる
...
例えば、ボタンに影がついていたり、スワイプでページを変えるときにめくっているような感じがあったり現実世界に近い感覚で操作ができるデザイン　`MaterialApp`を指定することで簡単に再現できます。詳しくは「マテリアルデザインとは」で検索してみよう。

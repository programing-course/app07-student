...
2023年現在、日本の消費税は食品に分類されるものが8%、それ以外は10%となっています。
...
- 変数`time`に登校時刻が例で代入されている（10時）
- `check`関数を呼び出す　引数は`time`
- check関数の処理が下記になるようにコードを追加しよう
- 10時以前なら「おはよう！」
- それ以外なら「遅刻に気をつけよう！」
...
```
変数`time`の値を変更して正しく出力されることを確認しましょう
```
>> 遅刻に気をつけよう!
>> おはよう! 
```
